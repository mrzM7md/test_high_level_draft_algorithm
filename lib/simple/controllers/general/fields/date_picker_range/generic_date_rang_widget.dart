...
// قم باستيراد GenericDateStyle هنا
...
class GenericDateWidget extends StatelessWidget {
  ...
  const GenericDateWidget({
    ...
    this.hintText = "اختر التاريخ...", this.firstDate, this.lastDate,
    this.style, this.customBuilder,
  });
      ...
      builder: (context, _) {
        ...
        if (temp != null) {
          suffixActions = IconButton(
            ...
            onPressed: () => controller.clear(), splashRadius: 20,
          );
        }
        ...
        return Padding(
          ...
          child: InputDecorator(
            decoration: appliedStyle.decoration!.copyWith(
              labelText: labelText, errorText: controller.validationError,
...
class GenericDateRangeWidget extends StatelessWidget {
  ...
  const GenericDateRangeWidget({
    super.key, required this.controller, required this.labelText,
    this.fromLabelText = "من", this.toLabelText = "إلى",
    this.firstDate, this.lastDate, this.style, this.customBuilder,
  });

  // 🚀 السحر هنا: استخدام نافذة واحدة موحدة للمدى الزمني!
  Future<void> _pickDateRange(BuildContext context) async {
    ...
    final pickedRange = await showDateRangePicker(
      ...
      firstDate: firstDate ?? DateTime(2000),
      lastDate: lastDate ?? DateTime(2100),
      builder: (context, child) => Directionality(textDirection: Directionality.of(context), child: child!),
    );
      ...
      builder: (context, _) {
        ...
        return Padding(
          ...
          child: InputDecorator(
            decoration: appliedStyle.decoration!.copyWith(
              labelText: labelText, errorText: controller.validationError,
              suffixIcon: temp != null ? IconButton(icon: Icon(Icons.close_rounded, color: Theme.of(context).colorScheme.error, size: 20), onPressed: () => controller.clear()) : null,
            ),
            child: InkWell(
              ...
              child: Row(
                children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text(temp?.fromDate != null ? appliedStyle.dateFormatter!(temp!.fromDate) : "---", style: appliedStyle.textStyle)
                  ])),
                  Container(width: 1, height: 30, color: Theme.of(context).dividerColor), const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                    Text(toLabelText, style: appliedStyle.labelStyle),
                    Text(temp?.toDate != null ? appliedStyle.dateFormatter!(temp!.toDate) : "---", style: appliedStyle.textStyle)
                  ])),
